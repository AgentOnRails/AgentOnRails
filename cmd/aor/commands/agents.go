...
type chainOption struct {
	...
	caip2   string
	testnet bool
	// keyType is the rails.x402.key_type this chain needs — empty means
	// ecdsa (the field's own default, so it's simply omitted from the
	// written config rather than written out as "ecdsa" explicitly).
	keyType string
}
...
var chainOptions = []chainOption{
	...
	{"Solana mainnet   ", "solana:5eykt4UsFv8P8NJdTREpY1vzqKqZKvdp", false, x402.KeyTypeEd25519},
	{"Solana devnet    ", "solana:EtWTRABZaYq6iMfeYKouRu166VU2xqa1", true, x402.KeyTypeEd25519},
}
...
var agentsCreateCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		// Chain selection — asked before the wallet address so the address
		// prompt below can hint the right format (0x... vs base58) for
		// whichever chain family was picked.
		fmt.Println()
		...
		// Wallet address
		addrHint := "Wallet address (0x...)"
		if chain.keyType == x402.KeyTypeEd25519 {
			addrHint = "Wallet address (base58)"
		}
		walletAddr := prompt(in, addrHint, "")
		if walletAddr == "" {
		...
		yaml := buildAgentYAML(agentID, port, walletAddr, chain.caip2, chain.keyType, dailyLimit, perCallMax, endpointMode)
...
func buildAgentYAML(agentID string, port int, walletAddr, chain, keyType, daily, perCall, endpointMode string) string {
	...
	// key_type is only written when it isn't the field's own default
	// (ecdsa) — every pre-Solana agent config keeps looking exactly as it
	// did before this option existed.
	keyTypeLine := ""
	if keyType != "" && keyType != x402.KeyTypeECDSA {
		keyTypeLine = fmt.Sprintf("    key_type: %q\n", keyType)
	}
...
`,
		...
		keyTypeLine, dailyLine, perCallLine,
		endpointMode, chain,
	)
}
